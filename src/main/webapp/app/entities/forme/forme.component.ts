...
export class FormeComponent implements OnInit, OnDestroy {
  ...
  trackId(index: number, item: IForme) {
    return item.id;
  }

  registerChangeInFormes() {
    this.eventSubscriber = this.eventManager.subscribe('formeListModification', response => this.reset());
  }
  ...
  protected paginateFormes(data: IForme[], headers: HttpHeaders) {
    this.links = this.parseLinks.parse(headers.get('link'));
    this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
    for (let i = 0; i < data.length; i++) {
      this.formes.push(data[i]);
    }
  }
